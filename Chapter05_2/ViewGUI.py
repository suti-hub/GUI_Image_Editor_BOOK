...
class ViewGUI():
    ...
    def __init__(self, window_root, default_path):
        ...
        self.speed_text       = ['x0.5','x1.0','x2.0','x3.0']
        ...
        self.window_video_canvas1 = tk.Canvas(self.tab2, height=192, width=454, bg='black')
        self.window_video_canvas2 = tk.Canvas(self.tab2, height=192, width=454, bg='black')
        ...
        label_cfg           = tk.Label(self.window_sub_ctrl2, text='[PreSet]')
        ...
        button_savecfg      = tk.Button(self.window_sub_ctrl2, text = 'Update',  width=5, command=self.event_savecfg)
        ...
        label_cfg.grid           (row=8, column=4, padx=5, pady=5, sticky=tk.W)
        button_savecfg.grid      (row=9, column=4, padx=5, pady=5, sticky=tk.W)
        ...
        load_cfg = self.control.LoadConfig()
        if load_cfg != None:
            self.select_tab = load_cfg['select_tab']
            self.control.SetTab(self.select_tab)
            tab = self.tab1 if self.select_tab == '[Photo]' else self.tab2
            self.notebook.select(tab)
            self.dir_path = self.control.GetDirPath()
    ...
    # Video
    def update_timestamp(self, is_status, total_frames, frame_no, h,m,s, video_tag):
        
        idx = int(video_tag.replace('Video','')) - 1
        if is_status:
            self.label_frame[idx]['text']   = 'Time:{:02}:{:02}:{:02}'.format(h,m,s)
            self.bar_position.set(int(frame_no/total_frames*100)) 
        else:
    ...
    def update_frameno(self, is_status, total_frames, frame_no, h,m,s, video_tag):
    ...
    def event_update_bar(self, val):
        ...
        if self.control.IsTransferToState('bar'):
    ...
    def event_play(self):
        ...
        if self.control.IsTransferToState('play'):
            ...
            if cur_state in ['EDIT', 'ECLIP', 'ELOCK']:
                self.set_message('Now editting.. To play video, set [Undo]')
    ...
    def event_speed(self):
        ...
        if self.control.IsTransferToState('speed'):
