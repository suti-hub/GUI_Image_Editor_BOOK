...
class ModelVideo(ModelImage):
    ...
    def draw_video(self, canvas, target_img, canvas_tag):
        ...
        self.pil_img              = Image.fromarray(img_conv)
        pil_img_pad               = ImageOps.pad(self.pil_img, (self.canvas_w, self.canvas_h))
    ...
    def draw_video_fast(self, canvas, target_img, canvas_tag):

        width, height             = self.canvas_w-2*self.pad_x, self.canvas_h
        cv2_img                   = cv2.resize(target_img, (width, height), cv2.INTER_CUBIC)
        img_conv                  = cv2.cvtColor(cv2_img, cv2.COLOR_BGR2RGB)
    ...
    def loop_video(self, loop=True):
        ...
        is_draw  = (self.cur_frame % self.skip_num == 0) or (loop == False)
        interval = self.interval if ((self.cur_frame+1) % self.skip_num == 0) else self.skip_interval

        if loop and self.ret:
            self.vid = self.canvas_video.after(interval, self.loop_video)
        ...
        if self.ret:
            ...
            if is_draw:
                self.draw_video_fast(self.canvas_video, self.video_img, self.canvas_tag)
    ...
    def set_interval(self, speed):
        ...
        with self.lock:
            ...
            val                = self.base_tick*1/speed
            self.interval      = int(0.9*val)
            self.skip_interval = int(0.7*val)
            if self.interval < self.interval_limit:
                self.interval = self.interval_limit
            
            print(f'{self.interval:.2f}, {self.skip_interval:.2f}, {self.skip_num}')
    
    
    def save_capture(self):
        
        if self.pil_img == None:
            img_conv      = cv2.cvtColor(self.video_img, cv2.COLOR_BGR2RGB)
            self.pil_img  = Image.fromarray(img_conv)
        
        name, ext     = os.path.splitext(self.file_name)
        file_path     = '{}/{}_{:05}.png'.format(self.output_path, name, self.cur_frame)        
        self.pil_img.save(file_path)
        print("[{}] Saved: {}".format(self.canvas_tag, file_path))
